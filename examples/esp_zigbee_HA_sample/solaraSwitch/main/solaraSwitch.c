...
static void zb_buttons_handler(switch_func_pair_t *button_func_pair)
{
    ...
    case SWITCH_ONOFF_TOGGLE_CONTROL: {
        ...
        cmd_level.zcl_basic_cmd.dst_addr_u.addr_short = on_off_light.short_addr;
        cmd_level.zcl_basic_cmd.dst_endpoint = on_off_light.endpoint;
        cmd_level.zcl_basic_cmd.src_endpoint = HA_ONOFF_SWITCH_ENDPOINT;
        //cmd_req.address_mode = ESP_ZB_APS_ADDR_MODE_16_ENDP_PRESENT;

        esp_zb_lock_acquire(portMAX_DELAY);
        esp_zb_zcl_level_move_to_level_with_onoff_cmd_req(&cmd_level);
        esp_zb_lock_release();
        ESP_EARLY_LOGI(TAG, "Sending moving light command to address(0x%x) endpoint(%d) %d level", on_off_light.short_addr, on_off_light.endpoint, level_value);
        level_value += step;
        ...
        esp_zb_lock_release();
        ESP_EARLY_LOGI(TAG, "Sending 'on_off toggle' command to address(0x%x) endpoint(%d)", on_off_light.short_addr, on_off_light.endpoint);
        */
    } break;
    default:
        break;
...
static void esp_zb_task(void *pvParameters)
{
    ...
    esp_zb_cluster_list_add_level_cluster(esp_zb_cluster_list, esp_zb_level_cluster, ESP_ZB_ZCL_CLUSTER_CLIENT_ROLE);
